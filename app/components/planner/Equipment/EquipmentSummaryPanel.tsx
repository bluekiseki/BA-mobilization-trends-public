...
export const EquipmentSummaryPanel: React.FC<EquipmentSummaryPanelProps> = ({ items, emptyText, eventDataForIcon, iconData, iconInfoData, panelClassName }) => {
  if (items.length === 0) {
    return <div className={`px-3 py-4 text-center text-xs text-gray-400 dark:text-gray-500 ${panelClassName}`}>{emptyText}</div>;
  }

  return (
    <div className={`flex flex-wrap gap-1.5 px-3 py-2 max-h-64 overflow-y-auto ${panelClassName}`}>
