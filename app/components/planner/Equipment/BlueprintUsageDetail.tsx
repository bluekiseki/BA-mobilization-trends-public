...
export const BlueprintUsageDetail: React.FC<BlueprintUsageDetailProps> = ({ blueprintsUsedByItem, blueprintsUsed, eventDataForIcon, iconData }) => {
  ...
  return (
    <div className="shrink-0 border-t border-gray-100 dark:border-neutral-800 px-3 py-2">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">{t('equipment.blueprintUsageDetail')}</p>
        ...
        return (
          <div key={eqType} className="mb-2">
            ...
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className="text-[11px] font-semibold text-gray-600 dark:text-gray-300">{eqType}</span>
              <span className="font-mono text-[10px] text-gray-400 dark:text-gray-500 ml-1">{t('equipment.blueprintTotalUsed', { count: totalUsed })}</span>
            </div>
            {/* Details per item */}
              ...
              {Object.entries(itemMap).map(([key, usedBp]) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-0.5 bg-yellow-50 dark:bg-yellow-900/20 rounded px-1.5 py-0.5">
                    ...
                    <span className="text-[9px] text-gray-400 dark:text-gray-500">{t('equipment.blueprintSheets', { count: usedBp })}</span>
                  </div>
                );
              })}
            </div>
