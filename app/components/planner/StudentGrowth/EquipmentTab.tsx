...
export const EquipmentTab = ({ plan, studentInfo, handleBatchUpdate, iconData, eventData }: EquipmentTabProps) => {
  ...
  const renderTierInfo = (tier: number, equipType: string, isInvalid: boolean, onChange: (val: number) => void) => {
    const equipId = getEquipmentId(equipType, tier);
    const iconBase64 = equipId && iconData?.Equipment?.[equipId];
    const colorClass = isInvalid ? 'text-rose-500' : tier > 0 ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500';
    ...
    return (
      <div className="flex items-center gap-1">
        {iconBase64 ? (
          ...
          <div className="w-6 h-6 bg-gray-200/50 dark:bg-neutral-800/50 rounded-md" />
        )}
        ...
        return (
          <div key={index} className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between">
              <span className="font-bold text-sm text-gray-800 dark:text-gray-100 tracking-wide uppercase">{t(`common.${equipType}`, equipType)}</span>
