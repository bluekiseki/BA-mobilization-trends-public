...
const InventoryCounter = ({ value, onChange, needed }: { value: number; onChange: (v: number) => void; needed?: number }) => {
  ...
  const hintColor = fulfilled ? 'text-green-500' : partial ? 'text-red-400' : 'text-gray-400 dark:text-neutral-500';

  return (
    <div className="flex flex-col items-center gap-0.5">
      <div className="flex items-center h-7 rounded overflow-hidden border border-gray-300 dark:border-neutral-600 text-xs">
        <button
          {...decEvents}
          tabIndex={-1}
          className="w-7 h-full bg-gray-100 dark:bg-neutral-700 hover:bg-gray-200 dark:hover:bg-neutral-600 active:bg-gray-300 dark:active:bg-neutral-500 font-bold select-none touch-manipulation"
        >
          −
        </button>
        <CustomNumberInput
          ...
          min={0}
          placeholder="0"
          className="w-11 h-full text-xs bg-white dark:bg-neutral-800 dark:text-gray-200 focus:bg-blue-50 dark:focus:bg-neutral-700"
        />
        <button
          {...incEvents}
          tabIndex={-1}
...
// ── Sort bar ────────────────────────────────────────────────────────────────
const SortBar = ({ sortBy, onChange, t }: { sortBy: 'rarity' | 'id'; onChange: (s: 'rarity' | 'id') => void; t: (k: any) => string }) => (
  <div className="flex items-center gap-2 px-4 py-1.5 border-b border-gray-100 dark:border-neutral-800 shrink-0">
    <span className="text-[11px] text-gray-400 dark:text-neutral-500">{t('ui.sortLabel')}</span>
    {(['rarity', 'id'] as const).map((s) => (
      <button
        key={s}
        onClick={() => onChange(s)}
        className={`text-[11px] px-2 py-0.5 rounded-full border transition-colors ${sortBy === s ? 'bg-blue-500 text-white border-blue-500' : 'text-gray-500 dark:text-gray-400 border-gray-300 dark:border-neutral-600 hover:border-blue-400'}`}
      >
...
export const UnifiedMaterialModal = ({ isOpen, onClose, calculatedNeeds, eventData, iconData, title, initialTab = 'needs' }: UnifiedMaterialModalProps) => {
  ...
  const TAB_INACTIVE = 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200';
  ...
  return (
    <div className="fixed inset-0 z-200 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      ...
      >
        {/* Header tabs */}
        <div className="flex items-center justify-between px-4 pt-2 border-b border-gray-200 dark:border-neutral-700 shrink-0 bg-white dark:bg-neutral-800">
          <div className="flex gap-0.5">
            <button className={`${TAB_BASE} ${activeTab === 'needs' ? TAB_ACTIVE : TAB_INACTIVE}`} onClick={() => setActiveTab('needs')}>
              ...
              <span className="ml-1.5 text-xs bg-gray-100 dark:bg-neutral-700 text-gray-500 dark:text-gray-400 rounded-full px-1.5 py-0.5">{needsItemCount}</span>
            </button>
            <button className={`${TAB_BASE} ${activeTab === 'inventory' ? TAB_ACTIVE : TAB_INACTIVE}`} onClick={() => setActiveTab('inventory')}>
              {t('ui.ownedInput')}
            </button>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xl leading-none px-2 pb-2">
            ×
          </button>
        </div>
        ...
        {activeTab === 'inventory' && (
          <>
            <SortBar sortBy={sortBy} onChange={setSortBy} t={t} />
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-2 px-4 py-2 border-b border-gray-100 dark:border-neutral-800 shrink-0">
              <div className="flex gap-1">
                <button
                  onClick={() => setFilterAll(true)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${filterAll ? 'bg-blue-500 text-white border-blue-500' : 'text-gray-500 dark:text-gray-400 border-gray-300 dark:border-neutral-600 hover:border-blue-400'}`}
                >
              ...
              <input
                ...
                className="flex-1 min-w-0 max-w-xs h-7 px-2.5 text-xs rounded-full border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 dark:text-gray-200 focus:outline-none focus:border-blue-400"
              />
              ...
              <div className="flex items-center gap-1 ml-auto">
                {confirmReset ? (
                  <>
                    <span className="text-xs text-gray-500 dark:text-gray-400">{t('ui.inventoryResetConfirm')}</span>
                    <button onClick={() => setConfirmReset(false)} className="text-xs px-2 py-0.5 rounded border border-gray-300 dark:border-neutral-600 text-gray-500 dark:text-gray-400">
                  ...
                  <button
                    ...
                    className="text-xs px-2.5 py-1 rounded-full border border-gray-300 dark:border-neutral-600 text-gray-500 dark:text-gray-400 hover:border-red-400 hover:text-red-500 transition-colors"
                  >
                    {t('ui.inventoryReset')}
                  </button>
                )}
            ...
            {/* Progress bar */}
            <div className="h-0.5 bg-gray-100 dark:bg-neutral-800 shrink-0">
            ...
            <div className="overflow-y-auto flex-1 px-4 py-4 space-y-5">
              {inventoryGroups.length === 0 ? (
                <p className="text-center text-sm text-gray-400 dark:text-neutral-500 py-8">{t('ui.inventorySearchEmpty')}</p>
              ) : (
                inventoryGroups.map(({ categoryKey, items }, groupIdx) => {
                  ...
                  return (
                    <div key={categoryKey}>
                      {groupIdx > 0 && <div className="border-t border-gray-100 dark:border-neutral-800 mb-4" />}
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-xs font-semibold text-gray-400 dark:text-neutral-500 uppercase tracking-wide">{tStr(CATEGORY_I18N[categoryKey] ?? categoryKey)}</div>
