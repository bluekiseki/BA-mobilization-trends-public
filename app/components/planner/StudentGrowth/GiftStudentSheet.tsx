...
export const GiftStudentSheet = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onMouseDown={onClose}>
      ...
      >
        ...
        <div className="sm:hidden flex justify-center pt-2.5 pb-1 shrink-0">
          <div className="w-8 h-1 rounded-full bg-gray-300 dark:bg-neutral-700" />
        </div>

        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-neutral-800 shrink-0">
          <div className="relative w-10 h-10 bg-gray-50 dark:bg-neutral-800 rounded-lg flex items-center justify-center shrink-0 overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800 text-gray-500 hover:bg-gray-200 dark:hover:bg-neutral-700 transition-colors shrink-0 text-sm font-bold"
          >
            ✕
          </button>
        </div>
        ...
        <div className="overflow-y-auto custom-scrollbar px-3 py-3 flex flex-col gap-4">
          {grouped.length === 0 ? (
            <p className="text-center text-sm text-gray-400 py-8">{t('affectionTab.giftIndex.noStudents')}</p>
          ) : (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {group.map((s) => (
                    <Tooltip key={s.id} placement="top" overlay={<span className="text-xs font-medium">{s.name}</span>} mouseEnterDelay={0.1}>
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 dark:bg-neutral-700 ring-2 ring-white dark:ring-neutral-800 cursor-pointer">
                        {studentPortraits?.[Number(s.id)] ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-xs font-bold text-gray-500">{s.name[0]}</div>
                        )}
                      </div>
                    </Tooltip>
                  ))}
