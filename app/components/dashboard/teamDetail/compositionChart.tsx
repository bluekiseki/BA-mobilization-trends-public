...
export const CompositionChart: React.FC<{
  ...
  function processGroup(currentTeams: TeamSnapshot[], rank: number, compMap: Map<string, AggregatedComp>, excludeIncomplete: boolean) {
      ...
      <div className="bg-neutral-100 dark:bg-neutral-800 p-3 rounded-lg text-sm flex flex-col gap-3">
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex bg-white dark:bg-neutral-900 rounded p-1 border dark:border-neutral-700">
            {(['team', 'report'] as const).map((mode) => (
              <button key={mode} onClick={() => setAnalysisUnit(mode)} className={`px-3 py-1 rounded capitalize ${analysisUnit === mode ? 'bg-teal-500 text-white' : 'text-neutral-500'}`}>
          ...
          <div className="flex items-center gap-2 border-l pl-4 dark:border-neutral-700">
            <input type="checkbox" checked={filters.usePartyCount} onChange={(e) => setFilters((p) => ({ ...p, usePartyCount: e.target.checked }))} />
        ...
        {analysisUnit === 'team' && (
          <div className="flex items-center gap-2 pt-2 border-t dark:border-neutral-700">
            <input type="checkbox" checked={filters.useTeamIndex} onChange={(e) => setFilters((p) => ({ ...p, useTeamIndex: e.target.checked }))} />
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((p) => ({
                  ...p,
                  teamIndexDir: e.target.value as any,
                }))
              }
            ...
            >
                          ...
                          {char && (
                            <StudentIcon
                              character={
                                {
                                  id: char.id,
                                  isMulligan: topMul.mulliganIds.includes(char.id),
                                } as Character
                              }
                              student={studentData[char.id]}
                              portraitData={portraitData}
                            />
                          )}
                        </div>
              ...
              <div className="flex-1 w-full flex flex-col justify-center shrink-0 min-w-0 text-center md:text-left">
                <div className="flex justify-between items-baseline mb-0 mx-1.5 px-1">
                  <span className="text-neutral-700 dark:text-neutral-100 text-base font-bold">{((comp.totalCount / data.length) * 100).toFixed(1)}%</span>
