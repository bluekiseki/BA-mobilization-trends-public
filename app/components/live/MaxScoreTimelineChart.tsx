...
import { LineChart, Line, XAxis, YAxis, Label, Tooltip, ResponsiveContainer, Legend, ReferenceLine, type TooltipContentProps } from 'recharts';
...
import { type_translation, typecolor } from '../raid/raidToString';
...
export const MaxScoreTimelineChart: FC<MaxScoreTimelineChartProps> = ({ isRaid, timelineData, raidInfos, server }) => {
  ...
  const [axisType, setAxisType] = useState<'score' | 'time'>('time');
  ...
  const prevFilteredDataRef = useRef<(RaidTimelinePoint | ERaidTimelinePoint)[]>([]);
  ...
  const { chartData, resetTimestamps } = useMemo(() => {
    ...
    const data: ChartEntry[] = filteredTimelineData.map((timePoint) => {
  ...
  return (
    <div ref={chartContainerRef} className="w-full">
      ...
      <div className="flex flex-col gap-4 mb-4">
        <div className="flex flex-wrap justify-between items-center border-b border-neutral-200 dark:border-neutral-700 pb-3 gap-2">
          <div className="flex gap-1 rounded-lg bg-neutral-100 dark:bg-neutral-700 p-1">
            {raidInfos.length > 1 &&
              raidInfos.map((info, i) => (
                <button
                  key={`boss${i + 1}`}
                  onClick={() => setSelectedBoss(`boss${i + 1}` as 'boss1' | 'boss2' | 'boss3')}
            ...
            {isRaid && <span className="px-3 py-1 text-sm font-semibold text-neutral-500">Single Boss</span>}
          </div>

          <div className="flex gap-1 rounded-lg bg-neutral-100 dark:bg-neutral-700 p-1 ml-auto">
            <button
              onClick={() => setAxisType('score')}
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${axisType === 'score' ? 'bg-white dark:bg-neutral-900 shadow-sm' : ''}`}
            >
              Score
            </button>
            <button
              onClick={() => setAxisType('time')}
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${axisType === 'time' ? 'bg-white dark:bg-neutral-900 shadow-sm' : ''}`}
            >
              Time
            </button>
          </div>
      ...
      <ResponsiveContainer width="100%" height={500}>
        <LineChart data={chartData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
          ...
          <YAxis
            ...
            tickFormatter={(val: number) => (axisType === 'score' ? `${(val / 1e6).toFixed(2)}M` : formatTimeToTimestamp(val).split('.')[0])}
          ...
          <Tooltip
            content={(props) => {
              const { active, payload, label } = props as Partial<TooltipContentProps<number, string>>;
                    ...
                    {payload.map((p) => {
                      ...
                      return (
                        <p key={name} style={{ color }}>
                          <strong>{name}:</strong> {axisType === 'score' ? value.toLocaleString() : formatTimeToTimestamp(value)}
                        </p>
                      );
                    })}
                  </div>
