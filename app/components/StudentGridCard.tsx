...
const StatChange = ({
  ...
  return (
    <div className={`flex items-center justify-center gap-0.5 ${className}`}>
      ...
      {isChanged ? (
        <>
          <span className="text-gray-500">{labelFn(current)}</span>
          <span className="text-gray-300 mx-0.5 text-[8px]">→</span>
          <span className="text-blue-600 dark:text-blue-400 font-bold">{labelFn(target)}</span>
        </>
      ) : (
        <span className="text-gray-600 dark:text-gray-400">{labelFn(target)}</span>
      )}
    </div>
  );
};
...
export const StudentGridCard = ({
  ...
  const bulletColor = studentInfo?.BulletType
    ? (
        {
          Explosion: '#b62915',
          Pierce: '#bc8800',
  ...
  return (
    <div
      onClick={onClick}
      className="group relative flex flex-col bg-white dark:bg-neutral-800 border-x border-b border-gray-200 dark:border-neutral-700 rounded-lg p-2 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-pointer h-full"
      style={{
    ...
    >
      ...
      <div className="absolute top-1.5 left-1.5 flex items-center gap-1.5 z-20">
        <input
          ...
          className="w-3.5 h-3.5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer transition-transform hover:scale-110"
        />
        {}
      ...
      <button
        ...
        className="absolute top-1 right-1 p-1 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md transition-colors z-10 opacity-0 group-hover:opacity-100"
      ...
      <div className="flex flex-col items-center mb-1 mt-2">
        ...
        <div className="relative mb-1">
          {studentInfo && portraitBase64 ? (
            ...
            <div className="w-12 h-12 rounded-full bg-gray-100 dark:bg-neutral-700 flex items-center justify-center text-gray-400 text-xs">?</div>
          )}
        </div>
        {}
        <h3 className="text-xs font-bold text-gray-800 dark:text-gray-200 truncate w-full text-center px-1 mb-0.5">{studentInfo ? studentInfo.Name : '-'}</h3>
        {/* Rank (Stars) */}
      ...
      {/* Bottom: Stats Grid */}
      <div className="mt-auto flex flex-col gap-0.5 bg-gray-50 dark:bg-neutral-900/50 rounded p-1.5 text-[10px] font-mono leading-tight">
        {/* Row 1: Level & Affection */}
        <div className="flex justify-between items-center border-b border-gray-100 dark:border-neutral-700 pb-0.5 mb-0.5">
          {/* Level */}
        ...
        {/* Row 2: Skills */}
        <div className="flex justify-center border-b border-gray-100 dark:border-neutral-700 pb-0.5 mb-0.5">
