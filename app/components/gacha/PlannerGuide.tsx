...
const EXPANDED_ID = 'planner-guide-expanded';
const COLLAPSED_ID = 'planner-guide-collapsed';

// Pre-hydration script flips visibility to prevent flash during hydration.
const PREVENT_FLASH_SCRIPT = `try{if(localStorage.getItem(${JSON.stringify(GUIDE_STORAGE_KEY)})==='true'){var e=document.getElementById(${JSON.stringify(EXPANDED_ID)}),c=document.getElementById(${JSON.stringify(COLLAPSED_ID)});if(e)e.classList.add('hidden');if(c)c.classList.remove('hidden');}}catch(_){}`;

const monoStyle = { fontFamily: 'inherit' };

const NOTE_KEYS = ['note2', 'note3', 'note4', 'note5', 'note6', 'note7', 'note8', 'note9', 'note10'] as const;

interface Props {
  collapsed: boolean;
  onToggle: () => void;
  gachaTestPath: string;
}

export default function PlannerGuide({ collapsed, onToggle, gachaTestPath }: Props) {
  ...
  // Both states rendered; visibility toggle prevents hydration mismatch.
  return (
    <>
      <div id={COLLAPSED_ID} className={`mb-5 flex justify-start${collapsed ? '' : ' hidden'}`}>
        <button
          type="button"
          onClick={onToggle}
      ...
      <div id={EXPANDED_ID} className={`rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/60 px-4 py-3 mb-5${collapsed ? ' hidden' : ''}`}>
        {/* Header */}
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-700 pt-3">
          ...
          <ul className="space-y-1">
            ...
            <li className="flex gap-2 text-[11px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
              ...
              <span>
                {t('notes.note11_pre')}
                <Link to={gachaTestPath} className="text-blue-600 hover:underline dark:text-blue-400">
                  /planner/gacha-test
                </Link>
                {t('notes.note11_post')}
              </span>
      ...
      <script suppressHydrationWarning dangerouslySetInnerHTML={{ __html: PREVENT_FLASH_SCRIPT }} />
    </>
  );
}
