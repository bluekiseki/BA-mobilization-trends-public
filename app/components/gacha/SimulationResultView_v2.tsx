...
export default function SimulationResultView_v2({ result, initialPyroxenes, portraitMap, pyroxeneIcon, elephIconMap = {}, bankruptcyRate, strategies, allStudents }: Props) {
  ...
  const simpleBudgetOverrate = (() => {
    const bin = result.distCost.find((d) => d.binStart > initialPyroxenes);
    if (!bin) return 0;
  ...
  const p50bin = chartData.find((d) => d.cdf >= 50);
  const p90bin = chartData.find((d) => d.cdf >= 90);
  const p995bin = chartData.find((d) => d.cdf >= 99.5);

  // Trim right tail past P99.5 to remove truly-zero bins
  const trimmedChartData = (() => {
    const tailIdx = chartData.findIndex((d) => d.cdf >= 99.5);
    return tailIdx >= 0 ? chartData.slice(0, Math.min(tailIdx + 2, chartData.length)) : chartData;
  })();

  const renderCustomBar = (props: CustomBarProps) => {
    const entry = trimmedChartData[props.index];
    const isOverBudget = unit === 'pyroxenes' && entry?.binStart > budgetThreshold;
          ...
          <div className={`rounded-xl border px-4 py-4 flex items-center gap-4 ${bgCls}`}>
            ...
            <div className="flex flex-col items-end gap-3 shrink-0 text-right" style={monoStyle}>
              ...
              <div>
                ...
                <div className="text-base font-bold text-neutral-600 dark:text-neutral-300">{fmt(result.avgTotalCost)}</div>
                <div className="text-[10px] text-neutral-400 dark:text-neutral-500">
                  ≈{Math.round(result.avgTotalCost / 120).toLocaleString()}
                  {t('pulls_short')}
                </div>
              </div>
            </div>
            ...
            <div className="mb-2">
              <div className="bg-neutral-100 dark:bg-neutral-800 p-0.5 rounded-lg inline-flex gap-0.5">
                <button
                  ...
                  style={unit === 'pyroxenes' ? { background: '#77e0ff' } : {}}
                >
                  {pyroxeneIcon ? (
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={trimmedChartData} margin={{ top: 18, right: 4, left: 4, bottom: 4 }}>
                ...
                <XAxis dataKey="binStart" tick={{ fontSize: 10, fill: '#a3a3a3' }} tickFormatter={(val) => (unit === 'pyroxenes' ? `${(val / 1000).toFixed(0)}k` : `${val}`)} axisLine={false} />
            ...
            <div>
              ...
              <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
                ...
                <div className="max-h-52 overflow-y-auto custom-scrollbar">
                  {filtered.length === 0 ? (
                    <div className="py-6 text-center text-xs text-neutral-400">{t('search_no_result')}</div>
                  ) : (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <FaChevronDown size={8} className={`text-neutral-300 dark:text-neutral-600 transition-transform ${isSelected ? 'rotate-180' : ''}`} />
                              </div>
                            </div>
                          </button>
                        );
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={selected.elephDistribution} margin={{ top: 2, right: 0, left: 0, bottom: 2 }}>
                              ...
                              <Tooltip
                                ...
                                formatter={(value) => [typeof value === 'number' ? `${(value * 100).toFixed(1)}%` : value, t('probability_unit')]}
                              />
