...
export interface PlayerProps {
  song: Song | null;
  onClose: () => void;
  onSongEnd: (player: YouTubePlayer) => void;
...
const Player: React.FC<PlayerProps> = React.memo(({ song, onClose, onSongEnd, onTitleClick, onNextSong }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:bottom-4 md:right-4 md:left-auto md:w-100 md:rounded-xl md:overflow-hidden md:shadow-2xl">
      ...
      {/* Persistent bar */}
      <div className="relative bg-white/95 dark:bg-neutral-900/97 backdrop-blur-xl border-t border-neutral-200 dark:border-neutral-700/80 flex items-center gap-2 px-3 py-2">
        {/* Progress bar — collapsed only */}
        {
          /*! isExpanded && */ duration > 0 && (
            <div
              className="absolute top-0 left-0 right-0 h-0.5 bg-neutral-200 dark:bg-neutral-700 cursor-pointer group/progress"
              onClick={(e) => {
        ...
        <button
          ...
          className="shrink-0 p-1.5 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-500 dark:text-neutral-400 transition-colors"
        ...
        <div className="flex-1 min-w-0 cursor-pointer group" onClick={() => onTitleClick(song.id)}>
          <MarqueeText
            ...
            className="text-sm font-bold text-neutral-800 dark:text-neutral-100 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors"
          />
          <p className="text-[11px] text-neutral-400 dark:text-neutral-500 truncate">
            {`${Number(song.id) < 10000 ? '#' + song.id + ' ' : ''}${
              song.composer
                ?.split('/')
                .map((v) => '#' + v.trim().replace(/\s/gi, '_'))
                .join(' ') || ''
            }`}
          </p>
        </div>
        ...
        <button
          ...
          className="shrink-0 p-1.5 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 transition-colors"
        ...
        <button
          onClick={onClose}
          className="shrink-0 p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-neutral-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
          title="Close"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
