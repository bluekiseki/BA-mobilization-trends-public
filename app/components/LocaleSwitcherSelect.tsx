...
export function changePathLanguage(currentLocale: Locale, newLocale: Locale, pathname: string) {
  ...
  const isBlacklisted = BLACKLISTED_PATHS.some((path) => basePath.startsWith(path));
  let newPath;
...
export default function LocaleSwitcherSelect({ defaultValue, items, label }: LocaleSwitcherProps) {
  ...
  const [selectedItem, setSelectedItem] = useState<LocaleItem>(() => items.find((item) => item.value === defaultValue) || items[0]);
          ...
          <div className="py-1" role="none">
            {items.map((item) => (
              <button
                ...
                className={`${
                  selectedItem.value === item.value ? 'font-semibold bg-neutral-100 dark:bg-neutral-600 text-neutral-900 dark:text-white' : 'text-neutral-700 dark:text-neutral-300'
