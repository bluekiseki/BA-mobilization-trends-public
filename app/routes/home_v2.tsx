...
import { typecolor, type_translation_sorted } from '~/components/raidToString';
...
type EventStatus = 'active' | 'upcoming';
...
function getBossName(bossKey: string, locale: LocaleShortName): string {
  if (bossKey in bossData) return (bossData as any)[bossKey].name[locale] || bossKey;
  const shortBossKey = bossKey.split('_')[0];
  if (shortBossKey in bossData) return (bossData as any)[shortBossKey].name[locale] || shortBossKey;
  return bossKey;
}
...
export async function loader({ context }: LoaderFunctionArgs) {
  ...
  let jpEvent: StatusEvent | null = null;
  {
    const events = Object.entries(eventListJson)
      .filter(([, d]) => (d as any).Planable !== false)
      .map(([id, d]) => ({
        id: Number(id),
        name: (d as any)[getlocaleMethond('', 'Jp', locale)] || (d as any).Jp || `Event ${id}`,
        openTime: new Date(formatInTimeZone((d as any).OpenTime)),
        closeTime: new Date(formatInTimeZone((d as any).CloseTime)),
      }));
  ...
  let glEvent: StatusEvent | null = null;
  {
    ...
    const processed = Object.entries(glDates).map(([idStr, dates]) => {
      ...
      const d = (eventListJson as any)[idStr];
      const name = d ? (d as any)[getlocaleMethond('', 'Jp', locale)] || d.Jp || `Event ${id}` : `Event ${id}`;
...
function ServerBadge({ server }: { server: 'JP' | 'GL/KR' }) {
...
const STATUS_BADGE_CLS: Record<StatusRaid['status'] | EventStatus, string> = {
  live: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
  active: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
  upcoming: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
};

function StatusBadge({ status }: { status: keyof typeof STATUS_BADGE_CLS }) {
...
function EventCard({ event, server, locale }: { event: StatusEvent | null; server: 'JP' | 'GL/KR'; locale: Locale }) {
  if (!event) return <EmptyCard server={server} />;

  return (
    <Link
      to={localeLink(locale, `/planner/event/${event.id}`)}
      className="group rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white/70 dark:bg-neutral-800/70 backdrop-blur-sm p-3.5 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-md transition-all flex flex-col gap-2 min-h-24"
    >
      <div className="flex items-center gap-1.5">
        <ServerBadge server={server} />
        <StatusBadge status={event.status} />
      </div>
      <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-100 line-clamp-2 leading-snug grow">{event.name}</p>
      <div className="flex items-center">
        <RemainingTime
          targetDate={new Date(event.dateIso)}
...
function RaidCard({ raid, server, locale }: { raid: StatusRaid | null; server: 'JP' | 'GL/KR'; locale: Locale }) {
  ...
  return (
    <Link
      to={to}
      className="group rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white/70 dark:bg-neutral-800/70 backdrop-blur-sm p-3.5 hover:border-violet-400 dark:hover:border-violet-500 hover:shadow-md transition-all flex flex-col gap-2 min-h-24"
    >
      {inner}
    </Link>
  );
...
function EmptyCard({ server }: { server: 'JP' | 'GL/KR' }) {
  const { t } = useTranslation('common');
  return (
...
function BirthdayWidget({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    let cancelled = false;
    import('~/data/birthdays.json').then(({ default: birthdaysJson }) => {
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    fetch(cdn('/w/students_portrait.json'))
      .then((r) => r.json() as Promise<StudentPortraitData>)
      .then((data) => {
        if (!cancelled) {
          setPortraits(data);
          setIsLoading(false);
        ...
        {displayEntries.map(({ student: s, dateKey, daysFromNow }) => {
          ...
          return (
            <Link key={`${s.id}-${dateKey}`} to={localeLink(locale, '/charts/jp/heatmap')} state={{ studentId: s.id }} className="flex flex-col items-center gap-1 shrink-0 group">
              <div
                className={`w-12 h-12 rounded-full overflow-hidden flex items-center justify-center border-2 transition-all group-hover:scale-105 group-hover:shadow-md ${
        ...
        <div className="space-y-3">
          {analyticsLinks.map((item) => (
            <Link
              key={String(item.to)}
              to={item.to}
              className="flex items-center gap-3 p-2.5 rounded-lg bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm transition-all group"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {toolCards.map((item) => (
            <Link
              key={String(item.to)}
              to={item.to}
              className="group flex flex-col rounded-xl p-2 border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm transition-all overflow-hidden"
            >
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative m-auto max-w-5xl px-6 md:py-24 py-20 text-center transition-colors duration-300">
      ...
      <div className="m-auto max-w-5xl px-4 pb-20 -mt-1 space-y-12">
        {/* Status Grid */}
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <RaidCard raid={jpRaid} server="JP" locale={locale as Locale} />
            <RaidCard raid={glRaid} server="GL/KR" locale={locale as Locale} />
            <EventCard event={jpEvent} server="JP" locale={locale as Locale} />
            <EventCard event={glEvent} server="GL/KR" locale={locale as Locale} />
          </div>
        </section>

        {/* Birthday */}
        <BirthdayWidget locale={locale as Locale} />
        ...
        <section>
          ...
          <ToolLinks locale={locale as Locale} isDark={isDark === 'dark'} />
        </section>
