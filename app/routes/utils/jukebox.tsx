      ...
      <div className="relative bg-white/80 dark:bg-neutral-800/80 backdrop-blur-md rounded-xl shadow-lg border border-neutral-200 dark:border-neutral-700">
        ...
        <div className={`pl-8 p-4 transition-opacity duration-300 ${isMinimized ? 'opacity-0' : 'opacity-100'}`}>
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="cursor-pointer group" onClick={() => onTitleClick(song.id)} title="Click to locate in list">
              <h3 className="font-mono text-xl font-bold text-blue-600 dark:text-blue-400 truncate group-hover:underline">{song.title ? song.title : `BGM ${song.id}`}</h3>
              <p className="text-sm text-neutral-600 dark:text-neutral-400 truncate">
                {`${Number(song.id) < 10000 ? '#' + song.id : ''} ${
                  song.composer
                    ?.split('/')
                    .map((v: string) => '#' + v.replace(/\s/gi, '_'))
                    .join(' ') || ''
                }`}
              </p>
            </div>
...
export default function JukeboxPage() {
  ...
  useEffect(() => {
    fetch(cdn('/w/bgm_database_combined.json'))
      .then((res) => res.json())
      .then((bgmData: any) => {
        setAllBgm(bgmData.sort((a: { id: any }, b: { id: any }) => Number(a.id) - Number(b.id)));
      });
  }, [setAllBgm]);
          ...
          bgm.xref.forEach((xref: any) => {
            if (xref.type === 'Main_Story' && xref.title) {
              ...
              if (mainStoryChapters[titleKey]) {
                ...
                mainStories.set(titleKey, t(`main_story.${mainStoryChapters[titleKey].key}` as any));
              } else {
            ...
            if (xref.type === 'Event_Story' && xref.title) {
              eventStories.set(
                String(xref.title),
                (!Number.isNaN(Number(xref.title))
                  ? eventJson[eventConvertor[Number(xref.title) as keyof typeof eventConvertor]]?.[{ en: 'En', ja: 'Jp', ko: 'Kr', 'zh-Hant': 'Tw' }[locale] as 'En' | 'Jp' | 'Kr']
                  : '') ||
  ...
  useEffect(() => {
    if (!isLoading && !error) {
      const activeGeneral = Object.entries(spoilerFilters.general)
        .filter(([, v]) => v)
        .map(([k]) => k);
      ...
      if (searchTerm) {
        ...
        results = results.filter((bgm: any) => bgm.id.includes(lowerSearchTerm) || bgm.title?.toLowerCase().includes(lowerSearchTerm) || bgm.composer?.toLowerCase().includes(lowerSearchTerm));
      }
  ...
  const handleToggleGlobalAll = () => {
    ...
    const allActive = allCategories.every((cat) => Object.values(spoilerFilters[cat]).every((val) => val === true));
    const newFilters = { ...spoilerFilters };
    allCategories.forEach((cat) => {
      const targetState = !allActive;
      const keys = Object.keys(newFilters[cat]);
      keys.forEach((key) => {
        newFilters[cat][key] = targetState;
      });
    });
  ...
  const handleStudentToggleAll = (type: 'favorStudents' | 'memorialStudents', filteredIds: string[]) => {
    ...
    filteredIds.forEach((id) => {
      newStudentFilters[id] = !allSelected;
    });
  ...
  const renderXrefLine = (xref: any, bgm: any): string => {
    ...
    switch (xref.type) {
      case 'Main_Story':
        ...
        const localizedMainTitle = mainStoryChapters[mainTitleKey] ? t(`main_story.${mainStoryChapters[mainTitleKey].key}` as any) : mainTitleKey;
      ...
      case 'Event_Story':
        const eventName =
          (!Number.isNaN(Number(xref.title)) &&
            eventData[eventConvertor[Number(xref.title) as keyof typeof eventConvertor]]?.[{ en: 'En', ja: 'Jp', ko: 'Kr', 'zh-Hant': 'Tw' }[locale] as 'En' | 'Jp' | 'Kr']) ||
          xref.title?.toString();
  ...
  if (error)
    return (
      <div className={`flex items-center justify-center min-h-screen font-sans ${pageBackground} text-red-500`}>
    ...
    <div className={`min-h-screen font-sans ${pageBackground}`}>
      <div className="max-w-7xl mx-auto p-2 sm:p-4 lg:p-8">
        <header className="mb-8 p-4 sm:p-0 ml-1">
          ...
          <p className="text-lg text-neutral-600 dark:text-neutral-300 max-w-3xl">{t('description')}</p>
        </header>
          ...
          <div className="md:col-span-1 bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md rounded-xl p-6 flex flex-col gap-5 self-start sticky top-6 shadow-lg border border-neutral-200 dark:border-neutral-700">
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={handleToggleGlobalAll} className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => setMainStoryModalOpen(true)}
                className="text-sm text-center py-2 px-3 bg-neutral-100 dark:bg-neutral-700/80 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-lg transition-colors text-neutral-700 dark:text-neutral-200"
              >
                {t('selection.main_story')}
                <span className="block text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  ({Object.values(spoilerFilters.mainStories).filter(Boolean).length}/{mainStoryList.length})
                </span>
              </button>
              <button
                onClick={() => setEventStoryModalOpen(true)}
                className="text-sm text-center py-2 px-3 bg-neutral-100 dark:bg-neutral-700/80 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-lg transition-colors text-neutral-700 dark:text-neutral-200"
              >
                {t('selection.event_story')}
                <span className="block text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  ({Object.values(spoilerFilters.eventStories).filter(Boolean).length}/{eventStoryList.length})
                </span>
              </button>
              <button
                onClick={() => setFavorStudentModalOpen(true)}
                className="text-sm text-center py-2 px-3 bg-neutral-100 dark:bg-neutral-700/80 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-lg transition-colors text-neutral-700 dark:text-neutral-200"
              >
                {t('selection.favor_story')}
                <span className="block text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  ({Object.values(spoilerFilters.favorStudents).filter(Boolean).length}/{studentFilterList.length})
                </span>
              </button>
              <button
                onClick={() => setMemorialStudentModalOpen(true)}
                className="text-sm text-center py-2 px-3 bg-neutral-100 dark:bg-neutral-700/80 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-lg transition-colors text-neutral-700 dark:text-neutral-200"
              >
                {t('selection.memorial')}
                <span className="block text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  ({Object.values(spoilerFilters.memorialStudents).filter(Boolean).length}/{studentFilterList.length})
                </span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(GENERAL_CATEGORIES) as Array<keyof typeof GENERAL_CATEGORIES>).map((key) => (
                <button
                  ...
                  className={`text-sm py-2 px-2 rounded-lg transition-colors ${spoilerFilters.general[key] ? 'bg-blue-600 text-white shadow-sm' : 'bg-neutral-100 dark:bg-neutral-700/80 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-300'}`}
                >
            ...
            <div className="flex flex-col gap-3 pt-2 border-t border-neutral-300 dark:border-neutral-700">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{showStoryNames ? t('actions.hide_story_names') : t('actions.show_story_names')}</span>
                <button
                  onClick={() => setShowStoryNames((prev) => !prev)}
                  className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${showStoryNames ? 'bg-blue-600' : 'bg-neutral-300 dark:bg-neutral-600'}`}
                >
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{t('playback.autoplay_scroll')}</span>
                <button
                  onClick={() => setAutoScrollEnabled((prev) => !prev)}
                  className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${isAutoScrollEnabled ? 'bg-blue-600' : 'bg-neutral-300 dark:bg-neutral-600'}`}
                >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-2 p-2 rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors text-neutral-600 dark:text-neutral-300"
                >
                  {playbackMode === 'autoplay' && (
                    <>
                      <AutoplayIcon /> <span className="text-sm font-medium">{t('playback.autoplay')}</span>
                    </>
                  )}
                  {playbackMode === 'repeat' && (
                    <>
                      <RepeatIcon /> <span className="text-sm font-medium">{t('playback.repeat_current')}</span>
                    </>
                  )}
                  {playbackMode === 'off' && (
                    <>
                      <StopIcon /> <span className="text-sm font-medium">{t('playback.stop_after')}</span>
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="w-full text-sm bg-neutral-500 hover:bg-neutral-600 dark:bg-neutral-600 dark:hover:bg-neutral-500 disabled:bg-neutral-300 dark:disabled:bg-neutral-700 disabled:text-neutral-500 text-white font-bold py-2 px-3 rounded-lg transition-colors"
                >
                ...
                <button
                  ...
                  disabled={filteredBgm.length === 0}
                  className="w-full text-sm bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 disabled:bg-neutral-300 dark:disabled:bg-neutral-700 disabled:text-neutral-500 text-white font-bold py-2 px-3 rounded-lg transition-colors"
                >
                    ...
                    >
                      <div className="cursor-pointer" onClick={() => setCurrentSong(bgm)}>
                        <div className="flex items-center justify-between">
                          ...
                          {bgm.youtube_url && (
                            <div className="flex items-center justify-center w-12 h-12 shrink-0 ml-4">
      ...
      {/* Modals */}
      <FilterModal
        isOpen={isMainStoryModalOpen}
        onClose={() => setMainStoryModalOpen(false)}
        title={t('selection.main_story')}
        items={mainStoryList}
        selectedItems={spoilerFilters.mainStories}
        onToggleItem={(id) => handleFilterToggle('mainStories', id)}
        onToggleAll={() =>
          handleToggleAll(
            'mainStories',
            mainStoryList.map(([id]) => id),
          )
        }
      />
      <FilterModal
        isOpen={isEventStoryModalOpen}
        onClose={() => setEventStoryModalOpen(false)}
        title={t('selection.event_story')}
        items={eventStoryList}
        selectedItems={spoilerFilters.eventStories}
        onToggleItem={(id) => handleFilterToggle('eventStories', id)}
        onToggleAll={() =>
          handleToggleAll(
            'eventStories',
            eventStoryList.map(([id]) => id),
          )
        }
      />
      <StudentFilterModal
        isOpen={isFavorStudentModalOpen}
        onClose={() => setFavorStudentModalOpen(false)}
        title={t('selection.favor_story')}
        items={studentFilterList}
        selectedItems={spoilerFilters.favorStudents}
        onToggleItem={(id) => handleFilterToggle('favorStudents', id)}
        onToggleAll={(filteredIds) => handleStudentToggleAll('favorStudents', filteredIds)}
      />
      <StudentFilterModal
        isOpen={isMemorialStudentModalOpen}
        onClose={() => setMemorialStudentModalOpen(false)}
        title={t('selection.memorial')}
        items={studentFilterList}
        selectedItems={spoilerFilters.memorialStudents}
        onToggleItem={(id) => handleFilterToggle('memorialStudents', id)}
        onToggleAll={(filteredIds) => handleStudentToggleAll('memorialStudents', filteredIds)}
      />
      ...
      <button
        onClick={() => {
          if (filterPanelRef.current)
            window.scrollTo({
              top: filterPanelRef.current.offsetTop - 60,
              behavior: 'smooth',
            });
        }}
