// app/routes/planner/Equipment_v2.tsx
...
import campaignIconInfo from '~/data/jp/campaign_icon_info.json';
import { createLinkHreflang, createMetaDescriptor } from '~/components/head';
import { getLocaleShortName, type Locale } from '~/utils/i18n/config';
import { getInstance } from '~/middleware/i18next';
import type { Route } from './+types/Equipment_v2';
...
function StatCard({ label, value, sub, accent, warn }: { label: string; value: string | number; sub?: string; accent?: boolean; warn?: boolean }) {
  return (
    <div className="bg-gray-50 dark:bg-neutral-800/60 rounded-md p-2.5 border border-gray-200 dark:border-neutral-700">
      <div className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">{label}</div>
      <div className={`font-mono text-xl font-bold leading-none ${accent ? 'text-blue-600 dark:text-blue-400' : warn ? 'text-amber-500 dark:text-amber-400' : 'text-gray-800 dark:text-gray-100'}`}>
        {value}
      </div>
      {sub && <div className="font-mono text-[10px] text-gray-400 dark:text-gray-500 mt-1">{sub}</div>}
    </div>
  );
}

// ─── Multiplier segmented control ──────────────────────────────────────────────
function MultiplierSeg({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex border border-gray-200 dark:border-neutral-700 rounded overflow-hidden text-[11px]">
      {[1, 2, 3].map((v) => (
        <button
          key={v}
          onClick={() => onChange(v)}
          className={`flex-1 py-1 transition-colors font-semibold ${
            value === v ? 'bg-blue-500 text-white' : 'bg-white dark:bg-neutral-800 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-neutral-700'
          }`}
        >
          {v}×
        </button>
...
function PaneHeader({ children }: { children: React.ReactNode }) {
  return (
    <div className="shrink-0 flex items-center gap-2 px-3 h-8 border-b border-gray-100 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/50">
      <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">{children}</span>
    </div>
  );
}
...
export default function EquipmentPlannerV2() {
  ...
  const [hardMode, setHardMode] = useState<'aggressive' | 'optimized' | 'manual'>('aggressive');
  ...
  useEffect(() => {
    ...
    fetch(cdn(`/ew/icon_info.json`))
      .then((r) => r.json() as any)
      .then((d) => setIconInfoData({ ...d, GachaGroup: { ...d.GachaGroup, ...campaignIconInfo.GachaGroup } }))
      .catch(console.error);
  }, [locale]);

  useEffect(() => {
    const path = campaignSource === 'jp' ? `/w/jp/campaigns.json` : `/w/kr/campaigns.json`;
    fetch(cdn(path))
      .then((r) => r.json() as any)
    ...
    return Object.entries(campaigns).map(([stageId, stageData]) => {
      ...
      stageData.Reward.forEach((reward) => {
        ...
        if (type === 'Equipment' || type === 'Item') {
          ...
          for (const [k, p] of Object.entries(gr)) drops[k] = (drops[k] || 0) + prob * p;
        }
      });
      return {
        id: Number(stageId),
  ...
  }, [hardMode, runCounts, resolvedStages, totalEquipmentNeeds, farmingDays, hardMultiplier, materialInventory, normalMultiplier, setRunCounts]);
    ...
    resolvedStages.forEach((s) =>
      Object.keys(s.drops).forEach((k) => {
        if (!['Equipment_1', 'Equipment_2', 'Equipment_3', 'Equipment_4'].includes(k)) {
          ...
          // Normalize blueprint equipment to standard equipment
          const normalized = type === 'Equipment' ? normalizeBluprintToEquipment(id) : id;
          items.add(`${type}_${normalized}`);
        }
      }),
    );
  ...
  const iconBtn =
    'flex items-center justify-center w-7 h-7 rounded border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors text-xs';

  // ─── RENDER ─────────────────────────────────────────────────────────────────
  return (
    <div className="flex flex-col overflow-hidden" style={{ height: 'calc(100dvh - 3.5rem)' }}>
      {/* ── TOPBAR ─────────────────────────────────────────────────────────── */}
      <div className="shrink-0 flex items-center gap-3 px-4 h-11 border-b border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-sm">
        <span className="font-bold tracking-tight dark:text-gray-100 truncate">{title}</span>
        ...
        <div className="flex gap-3 ml-auto text-[11px] text-gray-500 dark:text-gray-400 font-mono items-center flex-wrap justify-end">
          <span>
            <b className="text-gray-800 dark:text-gray-100">{selectedPlanUuids.size}</b>/{growthPlans.length} {t('equipment.students')}
          </span>
          {totalApUsed > 0 && (
            <span>
      ...
      {/* ── PANE TAB BAR ───────────────────────────────────────────────────── */}
      <div className="lg:hidden shrink-0 flex border-b border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900">
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-semibold border-b-2 transition-colors ${
              mobilePane === pane ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300'
            }`}
          >
            {icon}
            <span>{label}</span>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* ═══ LEFT: Input Pane ════════════════════════════════════════════════ */}
        <div
          className={`flex-col overflow-y-auto bg-white dark:bg-neutral-900 border-r border-gray-200 dark:border-neutral-700
          ${mobilePane !== 'input' ? 'hidden lg:flex' : 'flex'} w-full lg:flex-1`}
        >
          {/* Settings */}
          <div className="shrink-0 border-b border-gray-100 dark:border-neutral-800">
            <PaneHeader>{t('equipment.settings')}</PaneHeader>
            <div className="px-3 py-2 grid grid-cols-[72px_1fr] gap-x-2 gap-y-2 text-xs">
              <label className="text-gray-500 dark:text-gray-400 flex items-center">{t('equipment.source')}</label>
              <select
                value={campaignSource}
                onChange={(e) => setCampaignSource(e.target.value as 'kr' | 'jp')}
                className="w-full rounded border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 dark:text-gray-200 text-xs py-1 px-2 focus:outline-none focus:ring-1 focus:ring-blue-500"
              >
                <option value="kr">{t('equipment.sourceKR')}</option>
                <option value="jp">{t('equipment.sourceJPNew')}</option>
              </select>

              <label className="text-gray-500 dark:text-gray-400 flex items-center">{t('equipment.farmingDays')}</label>
              <NumberInput value={farmingDays} onChange={(v) => setFarmingDays(v || 1)} min={1} max={9999} />

              <label className="text-gray-500 dark:text-gray-400 flex items-center">{t('equipment.sourceLabelNormal')}</label>
              <MultiplierSeg value={normalMultiplier} onChange={(v) => setMultipliers('normal', v)} />

              <label className="text-gray-500 dark:text-gray-400 flex items-center">{t('equipment.sourceLabelHard')}</label>
              <MultiplierSeg value={hardMultiplier} onChange={(v) => setMultipliers('hard', v)} />
            </div>
          </div>

          {/* Student picker */}
          <div className="flex flex-col shrink-0 border-b border-gray-100 dark:border-neutral-800">
            <div className="shrink-0 flex items-center justify-between px-3 py-2 border-b border-gray-100 dark:border-neutral-800">
              <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                ...
                <span className="font-mono text-gray-600 dark:text-gray-300 normal-case tracking-normal ml-0.5">
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  className="text-[10px] px-2 py-0.5 rounded bg-gray-100 dark:bg-neutral-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-neutral-600"
                >
            ...
            <div className="shrink-0 flex gap-2 px-3 py-1.5 border-b border-gray-100 dark:border-neutral-800">
              <div className="relative flex-1">
                <FaSearch className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" style={{ fontSize: 10 }} />
                <input
                  type="text"
                  value={studentSearchQuery}
                  onChange={(e) => setStudentSearchQuery(e.target.value)}
                  placeholder={t('equipment.studentSearchPlaceholder')}
                  className="w-full pl-6 pr-5 py-1 text-xs border border-gray-200 dark:border-neutral-700 rounded bg-gray-50 dark:bg-neutral-800 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
                {studentSearchQuery && (
                  <button onClick={() => setStudentSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 text-sm leading-none">
                    ×
                  </button>
                )}
              </div>
              <button
                ...
                className={`text-[10px] font-semibold px-2 py-1 rounded whitespace-nowrap transition-colors ${
                  showOnlySelected ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-neutral-700 text-gray-600 dark:text-gray-400'
                }`}
              >
            ...
            <div className="overflow-y-auto p-2 max-h-48">
              {growthPlans.length === 0 ? (
                <div className="text-center py-8 text-xs text-gray-400 dark:text-gray-500 px-3">
                  <p className="font-semibold text-gray-600 dark:text-gray-300 mb-1">{t('ui.noStudentGrowthPlan')}</p>
                ...
                <>
                  ...
                  {filteredGrowthPlans.length === 0 && <p className="text-center text-xs text-gray-400 py-4">{t('equipment.studentSearchEmpty')}</p>}
                </>
              )}
            </div>

            <div className="shrink-0 px-3 py-2 border-t border-gray-100 dark:border-neutral-800">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-1 px-1.5 py-0.5 border-l-2 ${
                        status === 'full' ? 'border-green-400 dark:border-green-500' : status === 'deficit' ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-neutral-700'
                      }`}
                    >
                      ...
                      <span className="text-xs font-medium text-gray-600 dark:text-gray-400 truncate min-w-12">{t(`common.${type}`)}</span>
                      <CustomNumberInput
                        value={owned}
                        onChange={(e) => setBlueprint(type, Math.max(0, Math.min(9999, e || 0)))}
                        min={0}
                        max={9999}
                        className="w-12 text-center text-xs font-mono py-0.5 rounded border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-400"
                      />
        ...
        >
          {/* Normal / Hard tab bar */}
          <div className="shrink-0 flex items-center border-b border-gray-200 dark:border-neutral-700">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
                    stageFilter === type ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                  }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${type === 'Normal' ? 'bg-blue-400' : 'bg-red-400'}`} />
                  {type}
                  {count > 0 && <span className="font-mono text-[10px] text-gray-400 dark:text-gray-500">{count}</span>}
                </button>
              );
            })}
          ...
          {stageFilter === 'Hard' && hardElephStudents.length > 0 && (
            <div className="shrink-0 flex flex-wrap gap-1.5 items-center px-3 py-1.5 border-b border-gray-100 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/40">
              <span className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 shrink-0">{t('equipment.elephStudents')}</span>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setHardStudentFilter((prev) => {
                        const n = new Set(prev);
                        isSelected ? n.delete(studentId) : n.add(studentId);
                        return n;
                      })
                    }
                    title={student?.Name}
                    className={`w-7 h-7 rounded-full border-2 overflow-hidden shrink-0 transition-all ${
                      isSelected ? 'border-blue-500 ring-1 ring-blue-300 dark:ring-blue-700' : 'border-gray-200 dark:border-neutral-600 opacity-50 hover:opacity-100'
                    }`}
                  >
                    {portrait ? (
                      <img src={`data:image/webp;base64,${portrait}`} alt={student?.Name} className="w-full h-full object-cover" loading="lazy" />
                    ) : (
                      <div className="w-full h-full bg-gray-200 dark:bg-neutral-700 flex items-center justify-center text-[9px] text-gray-400">?</div>
                    )}
                  </button>
                );
              })}
              {hardStudentFilter.size > 0 && (
                <button onClick={() => setHardStudentFilter(new Set())} className="text-[10px] text-gray-400 hover:text-red-400 ml-1">
                  ✕
                </button>
              )}
            </div>
        ...
        {/* ═══ RIGHT: Results Pane ═════════════════════════════════════════════ */}
        <div
          className={`flex-col overflow-y-auto bg-white dark:bg-neutral-900 border-l border-gray-200 dark:border-neutral-700
          ${mobilePane !== 'results' ? 'hidden lg:flex' : 'flex'} w-full lg:flex-1`}
        >
          {/* Stat cards */}
          <div className="shrink-0 p-3 border-b border-gray-100 dark:border-neutral-800">
            <div className="grid grid-cols-2 gap-2">
              <StatCard label={t('equipment.statTotalAP')} value={totalApUsed.toLocaleString()} accent />
              <StatCard
          ...
          {/* Hard mode + Optimize */}
          <div className="shrink-0 p-3 border-b border-gray-100 dark:border-neutral-800">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">{t('equipment.hardModeTitle')}</p>
            <div className="grid grid-cols-3 gap-1.5 mb-3">
              {[
                { mode: 'aggressive' as const, label: t('equipment.hardModeAggressive'), desc: t('equipment.hardModeAggressiveDesc'), detail: t('equipment.hardModeAggressiveDetail') },
                { mode: 'optimized' as const, label: t('equipment.hardModeOptimized'), desc: t('equipment.hardModeOptimizedDesc'), detail: t('equipment.hardModeOptimizedDetail') },
                { mode: 'manual' as const, label: t('equipment.hardModeManual'), desc: t('equipment.hardModeManualDesc'), detail: t('equipment.hardModeManualDetail') },
              ].map(({ mode, label, desc }) => (
                <button
                  key={mode}
                  onClick={() => setHardMode(mode)}
                  className={`text-left rounded-md p-2 border-2 transition-all ${
                    hardMode === mode ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-200 dark:border-neutral-700 hover:border-gray-300 dark:hover:border-neutral-600'
                  }`}
                >
                  <div className={`text-[11px] font-bold leading-tight ${hardMode === mode ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200'}`}>{label}</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-tight">{desc}</div>
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={handleOptimize} className="flex-1 py-2 bg-green-500 hover:bg-green-600 text-white text-xs font-bold rounded-md transition-colors shadow-sm">
                {t('equipment.optimize')}
              </button>
              <button onClick={handleResetAll} className="flex-1 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-md transition-colors shadow-sm">
              ...
              {[
                ...
                <button
                  ...
                  className={`text-[11px] font-semibold px-2.5 py-1 rounded-md transition-colors ${
                    summaryTabMode === mode ? 'bg-blue-500 text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-neutral-800'
                  }`}
                >
                  {label}
                </button>
          ...
          {Object.values(runCounts).some((c) => c > 0) &&
            (() => {
              ...
              return (
                <div className="shrink-0 border-t border-gray-100 dark:border-neutral-800 px-3 py-2">
                  <div className="flex items-baseline gap-2 mb-1.5">
                    <p className="text-xs font-semibold text-gray-400 dark:text-gray-500">{t('equipment.farmingPlanTitle')}</p>
                    {normal.length > 0 && (
                    ...
                    {[...normal, ...hard].map((s) => {
                      ...
                      return (
                        <div key={s.id} className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-500 dark:text-gray-400 font-mono shrink-0">{isHard ? `H${s.chapter}-${s.stageNum}` : `N${s.chapter}-${s.stageNum}`}</span>
                          <b className={`text-sm font-mono shrink-0 ${isHard ? 'text-red-400' : 'text-blue-500'}`}>×{runCounts[s.id]}</b>
                          <span className="text-sm text-gray-400 dark:text-gray-500 font-mono shrink-0">{s.ap * (runCounts[s.id] || 0)}AP</span>
                          <span className="flex gap-0.5 ml-auto">
                            {topDrops.map(([k]) => {
                              const isItem = k.startsWith('Item_');
                              const itemId = String(k.split('_')[1]);
                              ...
                              return src ? (
                                ...
                                <EquipmentItemIcon
                                  ...
                                  itemId={String(k.split('_')[1])}
                                  size={28}
                                  imageOnly
                                  iconData={iconData}
                                  amount={''}
                                  eventData={{ icons: { Equipment: {} } } as any}
                                />
                              ) : // )
                              null;
                            })}
