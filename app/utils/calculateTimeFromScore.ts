...
const bosses_limit_4_30m = ['Yesod', '예소드', 'イェソド', 'EN0013'];
...
export function calculateScoreFromTime(time: number, difficulty: DifficultyName, boss: string, server: GameServer, id: string) {
  const timeout = getTimeoutFromBoss(boss);
  const needOldVersion = isNeedOldVersion(server, id);
